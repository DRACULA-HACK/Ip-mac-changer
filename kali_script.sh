...
CURRENT_MAC=$(ip link show dev eth0 | awk '/ether/ {print $2}')
...
NEW_MAC_VERIFY=$(ip link show dev eth0 | awk '/ether/ {print $2}')
...
NEW_IP_VERIFY=$(ip addr show dev eth0 | awk '/inet / {print $2}' | cut -d/ -f1)
